...
describe("Buffer.onUpdate", ({describe, _}) => {
  describe("normal mode", ({test, _}) => {
    test("add line", ({expect}) => {
        ...
        let dispose = Buffer.onUpdate((upd) => {
            updates := [upd, ...updates^];
        });
        ...
        expect.string(Array.get(bu.lines, 0)).toEqual("This is the second line of a test file");
  ...
  describe("insert mode", ({test, _}) => {
    test("single file", ({expect}) => {
        ...
        expect.int(Array.length(bu.lines)).toBe(1);
        expect.string(Array.get(bu.lines, 0)).toEqual("aThis is the first line of a test file");
        ...
        expect.int(bu.endLine).toBe(2);
        expect.int(Array.length(bu.lines)).toBe(1);
        expect.string(Array.get(bu.lines, 0)).toEqual("abThis is the first line of a test file");
