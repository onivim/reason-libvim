...
describe("CommandLine", ({describe, _}) => {
  /* TODO: Bring this test back and FIX! */
  ...
  describe("listeners", ({test, _}) => {
    test("enter / leave listeners", ({expect}) => {
        ...
        let expectCommandLineType = (m) => expect.bool(m == List.hd(enterEvents^).cmdType).toBe(true);
        let expectCommandLineText = (v) => expect.bool(String.equal(v , List.hd(enterEvents^).text)).toBe(true);

        let dispose1 = CommandLine.onEnter((c) => { enterEvents := [c, ...enterEvents^] });
    ...
    test("update listener", ({expect}) => {
        ...
        let expectCommandLineText = (v) => expect.bool(String.equal(v , List.hd(updateEvents^).text)).toBe(true);
        let expectCommandLinePosition = (v) => expect.bool(v == List.hd(updateEvents^).position).toBe(true);

        let dispose1 = CommandLine.onUpdate((c) => { updateEvents := [c, ...updateEvents^] });
